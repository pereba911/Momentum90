...
- Metas del mes / trimestre y comisiones con abonos parciales: lógica pura en `src/lib/commissions.ts`; UI en `HoyTab` (tarjetas de meta **de solo lectura**, celebración) y `CapitalTab` (área Metas: configuración + comisiones). Entidades persistidas en Supabase: `commissions` y `goalAdjustments`.
- **Dirección única de edición de metas (Metas → Hoy):** el monto objetivo y los reajustes se editan **solo** en el área Metas (`GoalsConfigSection` → `GoalEditor` → `applyGoalTarget`). El tab Hoy es un **espejo**: muestra la meta vigente, el avance y el estado, y sus tarjetas solo enlazan a Metas (`onManage` → `onGoTo("metas")`); `HoyTab` no importa ni invoca `applyGoalTarget`, no abre `GoalEditor` y nunca escribe `monthlyGoal`/`quarterlyGoal`/`goalAdjustments`. Regresión cubierta en `tests/commissions.test.ts` ("Hoy es un espejo de Metas").
