// ─────────────────────────────────────────────────────────────────────────────
// Reporte PDF de Gastos — Goal Assist 90
...
export function buildExpenseReportPdf(input: ExpenseReportInput): ExpenseReportResult {
  ...
  doc.text("Reporte de Gastos — Goal Assist 90", 14, y);
  y += 7;
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  ...
  for (let p = 1; p <= nPages; p++) {
    ...
    doc.text(`Goal Assist 90 · Reporte de gastos generado localmente · ${new Date().toISOString().slice(0, 10)}`, 14, pageH - 10);
